...
## Project Objective
Build and train a machine learning model to estimate the speed of a vehicle in real-time using only a forward-facing dashcam video.
*   **Input:** Live or pre-recorded dashcam video.
*   **Output:** Continuous live speed estimate (0 mph when stopped or moving <= 2mph).

---

## Phase 1: Data Acquisition
*Goal: Secure high-quality, synchronized video and speed data.*

We will use open-source driving datasets to avoid the cost and time of manual data collection.
1.  **Comma.ai Speed Prediction Challenge (Primary Start):** 26 minutes of driving at 20fps. Best for initial prototyping because it provides a perfectly synced `train.txt` file with exact speeds for every frame.
2.  **Comma2k19 (Scaling):** 33+ hours of highway driving with raw CAN bus speed logs for training a production-ready model.
3.  **BDD100K (Robustness):** Extreme weather and lighting conditions to test and correct model failures in rain or at night.
...
1.  **Extract Frames:** Use Python and OpenCV (`cv2.VideoCapture`) to break the `.mp4` video into individual image frames.
...
## Phase 4: Post-Processing & The "Stop" Rule
